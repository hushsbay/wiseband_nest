...
export class PhotoService {
    ...
    async qry(dto: Record<string, any>): Promise<any> {
        ...
        try {
            //const list = await this.photoRepo.find({ relations: { photoMeta: true } }) //정상 동작함
            const list = this.photoRepo.createQueryBuilder('photo').innerJoinAndSelect('photoMeta', 'photo').getMany() //데이터 못가져옴
